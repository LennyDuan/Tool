...
// get one object in a table
export
const list = async (payload, context, callback) => {
  console.log(JSON.stringify(payload));
  const event_security = payload.queryStringParameters.security;
  const event_table_name = payload.queryStringParameters.table_name;
  const uid = payload.queryStringParameters.uid;
  ...
  try {
    const data = await docClient.getItem(queryParameters).promise();
    callback(null, data)
  } catch(error) {
    console.log(error);
    callback(error, 'Unable to query this item')
  }
};
